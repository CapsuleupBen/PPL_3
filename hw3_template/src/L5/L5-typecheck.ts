...
export const typeofPrim = (p: PrimOp): Result<TExp> =>
    ...
    (p.op === 'cons') ?
        (tv => makeOk(makeProcTExp([tv, makeListTExp(tv)], makeListTExp(tv))))(T()) :
    (p.op === 'car') ? 
        (tv => makeOk(makeProcTExp([makeListTExp(tv)], tv)))(T()) :
    (p.op === 'cdr') ? 
        (tv => makeOk(makeProcTExp([makeListTExp(tv)], makeListTExp(tv))))(T()) :
...
export const typeofDefine = (exp: DefineExp, tenv: TEnv): Result<VoidTExp> => {
    ...
    return bind(valTypeResult, (valType: TExp) => {
        const constraintResult = checkEqualType(exp.var.texp, valType, exp);
        return bind(constraintResult, () => makeOk(makeVoidTExp()));
    });
};
...
const typeofProgramExps = (exps: Exp[], tenv: TEnv): Result<TExp> => {
    ...
    const f = exps[0];
    const r = exps.slice(1);

    if (exps.length === 1) {
        return isDefineExp(f) ? typeofDefine(f, tenv) : typeofExp(f, tenv);
    }

    if (isDefineExp(f)) {
        return bind(typeofDefine(f, tenv), () => {
            const extEnv = makeExtendTEnv([f.var.var], [f.var.texp], tenv);
            return typeofProgramExps(r, extEnv);
        });
    } else {
        return bind(typeofExp(f, tenv), () => typeofProgramExps(r, tenv));
    }
};