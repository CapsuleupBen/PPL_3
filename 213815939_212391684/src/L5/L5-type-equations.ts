...
export const makeEquationsFromExp = (exp: A.Exp, pool: Pool): Opt.Optional<Equation[]> =>
    ...
    A.isLitExp(exp) ?
        ...
        V.isCompoundSExp(exp.val) ?
            Opt.mapv(inPool(pool, exp), (left: T.TExp) => {
                const makeEqs = (val: V.SExpValue): {te: T.TExp, eqs: Equation[]} => {
                    if (V.isEmptySExp(val)) return {te: T.makeListTExp(T.makeFreshTVar()), eqs: []};
                    if (V.isCompoundSExp(val)) {
                        const head = makeEqs(val.val1);
                        const tail = makeEqs(val.val2);
                        return {
                            te: T.makeListTExp(head.te),
                            eqs: R.concat(head.eqs, R.concat(tail.eqs, [makeEquation(tail.te, T.makeListTExp(head.te))]))
                        };
                    }
                    if (isNumber(val)) return {te: T.makeNumTExp(), eqs: []};
                    if (isBoolean(val)) return {te: T.makeBoolTExp(), eqs: []};
                    if (isString(val)) return {te: T.makeStrTExp(), eqs: []};
                    return {te: T.makeVoidTExp(), eqs: []};
                };
                
                const res = makeEqs(exp.val);
                return cons(makeEquation(left, res.te), res.eqs);
            }) :
